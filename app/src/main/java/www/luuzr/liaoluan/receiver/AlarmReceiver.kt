                    ...
                    Intent.ACTION_DATE_CHANGED, Intent.ACTION_TIMEZONE_CHANGED -> {
                         ...
                         habits.forEach { h -> 
                             if (h.lastCompletedDate.isNotEmpty() && h.lastCompletedDate != today && h.progress > 0) {
                                 val isBroken = h.lastCompletedDate != yesterdayStr && h.lastCompletedDate != today
                                 val newStreak = if (isBroken) 0 else h.streakDays
                                 repository.updateHabit(
                                     h.copy(progress = 0, completed = false, streakDays = newStreak, actualStartTime = 0L)
                                 )
                             } else if (h.lastCompletedDate.isNotEmpty() && h.lastCompletedDate != today && h.completed) {
                                  val isBroken = h.lastCompletedDate != yesterdayStr
                                  repository.updateHabit(
                                      h.copy(progress = 0, completed = false, streakDays = if (isBroken) 0 else h.streakDays, actualStartTime = 0L)
                                  )
                             }
                         }
                    }
